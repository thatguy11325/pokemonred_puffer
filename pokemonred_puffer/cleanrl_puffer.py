...
def init(
    ...
    obs = torch.zeros(config.batch_size + 1, *obs_shape, device=obs_device)
    actions = torch.zeros(config.batch_size + 1, *atn_shape, dtype=int, device=device)
    logprobs = torch.zeros(config.batch_size + 1, device=device)
    rewards = torch.zeros(config.batch_size + 1, device=device)
    dones = torch.zeros(config.batch_size + 1, device=device)
    truncateds = torch.zeros(config.batch_size + 1, device=device)
    values = torch.zeros(config.batch_size + 1, device=device)
